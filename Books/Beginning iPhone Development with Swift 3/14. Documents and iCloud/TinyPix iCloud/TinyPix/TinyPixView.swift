...
class TinyPixView: UIView {
    ...
    private func toggleSelectedBlock() {
        if selectedBlockIndex.row != -1 && selectedBlockIndex.column != -1 {
            ...
            (document.undoManager?.prepare(withInvocationTarget: document!) as! TinyPixDocument).toggleStateAt(row: selectedBlockIndex.row, column: selectedBlockIndex.column)
            setNeedsDisplay()
        }
    }
